...
//         Pass table name as type, field as .field
        ...
        XCTAssertEqual(compile(q), "SELECT artists.name , artists.genre FROM artists")
    }
    func testFieldAlias() {
        let q = Select(Artist.f(.Name).alias("asd"), from: Artist.tableName)
        XCTAssertEqual(compile(q), "SELECT artists.name as asd FROM artists")
    }
    func testChainingSelect() {
        let q = Select(.Name, from: Artist.self).select("field", "otherfield")
        XCTAssertEqual(compile(q), "SELECT artists.name , field , otherfield FROM artists")
...
//        XCTAssertEqual(compile(q), "INSERT ")
    }
}
